...
class ArViewerViewManager : SimpleViewManager<ArViewerView>() {
  ...
  /**
   * Optional: resolvedAnchorId
   */
  @ReactProp(name = "resolvedAnchorId")
  fun resolvedAnchorId(view: ArViewerView, resolveId: ReadableArray?) {
    if(resolveId != null){
      val list = arrayListOf<String>()
      for (i in 0 until resolveId.size()) {
        list.add(resolveId.getString(i) ?: "")
      }
      Log.d("ARview resolveId", resolveId.toString())

    view.onSetCurrentModeResolve(list)
    }
